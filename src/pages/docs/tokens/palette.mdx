...
`--tl-{hue}-[number]` where hue is the generic hue name, and number is a range from 000 - 900 (or 000 - 300 for white and black) scaling in 100'semanticColors
...
**Tokens** provides the user with a 'semantic' palette. These are colors that are to have specific semantic meaning associated with them, these can be used in alerts, highlights or for button coloring to show meaning of certain actions.
...
Overidding the default palettes is a very simple step which doesnt require a build set up.
...
<ExampleCard>
  Step 1. - Add your own stylesheet linked in the head tag AFTER **Tokens**
</ExampleCard>
...
<ExampleCard>
  Step 2. - Add a `:root` pseudo element at the top of your stylesheet and
  override the tokens values.
</ExampleCard>
